+++
date = '2026-01-03T01:22:41+01:00'
draft = false
construction = false
title = 'Anhydrotetracycline Hydrochloride'
subtitle = ''
author = ['Spike Murphy Müller ¹']
affiliation = ['¹ Center for Experimental Medicine, Institute of Biochemistry and Signal Transduction, Working Group Signal Transduction in Cancer']
version = 'Version 1.0.0'
version_explanation = 'first: major revisions - second: minor revisions - third: revisions without changing procedure'
updated = '03.01.2026'
finished = true
tested = false

common_name = ['Anhydrotetracycline Hydrochloride']
short_name = ['AHT']
IUPAC = ['hydrogen (2Z)-2-[amino(hydroxy)methylidene]-4-(dimethylamino)-11,12a-dihydroxy-6-methyl-1,2,3,4,4a,5,12,12a-octahydrotetracene-1,3,12-trione chloride']
other_names = ['Anhydrotetracycline']
cas_number = ['13803-65-1']

formula = ['C<sub>22</sub>H<sub>23</sub>ClN<sub>2</sub>O<sub>7</sub>']
molecular_weight = ['462.88 g/mol']

signal_word = ['danger']
physical_hazard_classes = []
health_hazard_classes = [
  'acute toxicity',
  'serious eye damage',
  'eye irritation',
  'reproductive toxicity',
]
environmental_hazard_classes = []

gloves = ['nitrile gloves, EN ISO 374-1 type B']
eye_protection = ['recommended']
mask = ['not normally required; use dust mask if powder handling generates aerosols']
fume_hood = ['not required']

appearance = ['yellow crystalline solid']
melting_point = [
    'not determined',
    'decomposes'
]
boiling_point = [
    'not applicable',
    'decomposes before boiling'
]
solubility = [  
    '~5 g/L in water',
    'soluble in DMSO, ethanol, DMF, methanol'
]
pKa = []
ph_value = []
ph_range_effective = ['2-9']
acid_base_properties = ['amphoteric compound with multiple weakly acidic and basic functional groups']
net_charge = ['approximately neutral at pH 7']
isoelectric_point = []

storage_conditions = [
    '-20 °C', 
    'protected from light',
    'dry and tightly sealed',
    'optimally under inert gas (argon)'
]
stability = [
    'photosensitive',
    'moisture sensitive',
    'degrades upon prolonged exposure to light and moisture'
]
shelf_life = ['at least 1 year under proper storage conditions']
common_use_cases = [
    'transcriptional regulation in Tet-On/Tet-Off gene expression systems',
    'antibiotic research'
]
incompatibilities = [
      'strong acids',
      'strong bases bases',
      'strong oxidizing agents',
      'light exposure'
]
typical_stock_concentrations = [
    'from 21.6 µM (0.01 mg/mL)',
    'up to 4.32 mM (2 mg/mL)'
]
recipe = ['/recipes/bacteria/aht.md']

example_SDS = ['https://documents.thermofisher.com/DirectWebViewer/private/document.aspx?prd=ALFAAJ66688~~PDF~~MTR~~CLP1~~EE~~2024-02-11%2003:53:11~~Anhydrotetracycline%20hydrochloride~~']

references = [
  'National Center for Biotechnology Information. PubChem Compound Summary for CID 54710409, Anhydrotetracycline hydrochloride. https://pubchem.ncbi.nlm.nih.gov/compound/Anhydrotetracycline-hydrochloride. Accessed Jan. 3, 2026.', 
  'Sigma-Aldrich Chemie GmbH. Anhydrotetracyclin-hydrochlorid. https://www.sigmaaldrich.com/DE/de/product/sial/37919?srsltid=AfmBOoo8utA_y0POm8nBHRiS7kgy25xQSgU4LKniteyR93UMzrgMxi_O. Accessed Jan. 3, 2026.',
  'Life Technologies GmbH (Thermo Fisher Scientific Inc.). Anhydrotetracyclin hydrochloride. https://www.thermofisher.com/order/catalog/product/de/de/J66688.MA. Accessed Jan. 3, 2026.',
  'Fisher Scientific GmbH (Thermo Fisher Scientific Inc.). Anhydrotetracycline hydrochloride, Thermo Scientific™. https://www.fishersci.de/shop/products/anhydrotetracycline-hydrochloride-thermo-scientific/p-7057533?srsltid=AfmBOopy1LARyA61z5Uf4-NbmfyeQKcAjaio-wQ_5a2UXpyS3B1gngJ8. Accessed Jan. 3, 2026.',
  'IBA Lifesciences GmbH. Anhydrotetracycline. https://www.iba-lifesciences.com/de/anhydrotetracycline/2-0401-002. Accessed Jan. 3, 2026.']
+++

{{< biochemsheets/chemicals/biochemsheetbox type="info" title="METADATA" >}}
{{< biochemsheets/chemicals/chemicals_metadata >}}
{{< /biochemsheets/chemicals/biochemsheetbox >}}

{{< biochemsheets/chemicals/biochemsheetbox type="danger" title="SAFETY" >}}
{{< biochemsheets/chemicals/chemicals_safety >}}
{{< /biochemsheets/chemicals/biochemsheetbox >}}

{{< biochemsheets/chemicals/biochemsheetbox type="warning" title="HANDLING" >}}
{{< biochemsheets/chemicals/chemicals_handling >}}
{{< /biochemsheets/chemicals/biochemsheetbox >}}

{{< biochemsheets/chemicals/chemicals_references >}}

{{< biochemsheets/chemicals/chemicals_disclaimer >}}
